import { useEffect, useRef, useState } from "react";
import "./InfluencerMarketingWhyBrandsSend.css";

const FONT_HREF =
  "https://fonts.googleapis.com/css2?family=Krub:wght@300;400;500;600;700&display=swap";

const CHEVRON_BG = `url("data:image/svg+xml;utf8,${encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="%232D1963" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>',
)}")`;

const ROLES = ["Brand", "Agency", "Creator"];

const INDUSTRY = [
  "Select...",
  "CPG / Consumer goods",
  "Finance / Fintech",
  "Beauty / Wellness",
  "Travel / Hospitality",
  "Tech / SaaS",
  "Alcohol / Spirits",
  "Other",
];
const BUDGET = [
  "Select...",
  "Under $10k",
  "$10k - $25k",
  "$25k - $50k",
  "$50k - $100k",
  "$100k+",
];
const COMPANY_SIZE = [
  "Select...",
  "1-10 employees",
  "11-50 employees",
  "51-200 employees",
  "201-1000 employees",
  "1000+ employees",
];
const COUNTRY = [
  "Select...",
  "United States",
  "United Kingdom",
  "Canada",
  "Australia",
  "Germany",
  "France",
  "Other",
];

const BENEFITS = [
  {
    t: "Reply within 1 business day",
    d: "Real reply from a campaign strategist, not an SDR auto-responder.",
  },
  {
    t: "Custom proposal in 48 hours",
    d: "Three budget tiers, creator shortlist preview, projected reach.",
  },
  {
    t: "No sales pressure",
    d: "If it's not a fit, we'll tell you. Sometimes a marketplace is the right call.",
  },
];

export default function SocialMeloBriefForm() {
  const [role, setRole] = useState("Brand");
  const [inView, setInView] = useState(false);
  const ref = useRef(null);

  useEffect(() => {
    if (typeof document === "undefined" || document.getElementById("smh-fonts"))
      return;
    const pre1 = Object.assign(document.createElement("link"), {
      rel: "preconnect",
      href: "https://fonts.googleapis.com",
    });
    const pre2 = Object.assign(document.createElement("link"), {
      rel: "preconnect",
      href: "https://fonts.gstatic.com",
      crossOrigin: "anonymous",
    });
    const link = Object.assign(document.createElement("link"), {
      id: "smh-fonts",
      rel: "stylesheet",
      href: FONT_HREF,
    });
    document.head.append(pre1, pre2, link);
  }, []);

  useEffect(() => {
    const el = ref.current;
    if (!el || typeof IntersectionObserver === "undefined") {
      setInView(true);
      return;
    }
    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            setInView(true);
            io.disconnect();
          }
        });
      },
      { threshold: 0.15 },
    );
    io.observe(el);
    return () => io.disconnect();
  }, []);

  const Label = ({ children }) => (
    <span className="tu-label">
      {children}
      <span className="tu-req">*</span>
    </span>
  );

  return (
    <section className={"tu" + (inView ? " in" : "")} ref={ref}>
      <div className="tu-wrap">
        {/* form */}
        <div className="tu-card tu-rev">
          <h2 className="tu-title">Tell us about your campaign</h2>
          <p className="tu-lead">
            60-second form. We reply within 1 business day with next steps.
          </p>

          <div className="tu-grid2">
            <label className="tu-field">
              <Label>First Name</Label>
              <input className="tu-input" type="text" />
            </label>
            <label className="tu-field">
              <Label>Last Name</Label>
              <input className="tu-input" type="text" />
            </label>
            <label className="tu-field">
              <Label>Work Email</Label>
              <input
                className="tu-input"
                type="email"
                placeholder="you@company.com"
              />
            </label>
            <label className="tu-field">
              <Label>Company</Label>
              <input className="tu-input" type="text" />
            </label>
          </div>

          <div className="tu-grid2" style={{ marginTop: "20px" }}>
            <label className="tu-field">
              <Label>Company Size</Label>
              <select
                className="tu-select"
                defaultValue="Select..."
                style={{ backgroundImage: CHEVRON_BG }}
              >
                {COMPANY_SIZE.map((o) => (
                  <option key={o}>{o}</option>
                ))}
              </select>
            </label>
            <label className="tu-field">
              <Label>Country</Label>
              <select
                className="tu-select"
                defaultValue="Select..."
                style={{ backgroundImage: CHEVRON_BG }}
              >
                {COUNTRY.map((o) => (
                  <option key={o}>{o}</option>
                ))}
              </select>
            </label>
          </div>

          <div className="tu-block">
            <span className="tu-label">
              I'm a...<span className="tu-req">*</span>
            </span>
            <div className="tu-seg" role="group" aria-label="I am a">
              {ROLES.map((r) => (
                <button
                  key={r}
                  type="button"
                  className={"tu-seg-btn" + (role === r ? " on" : "")}
                  aria-pressed={role === r}
                  onClick={() => setRole(r)}
                >
                  {r}
                </button>
              ))}
            </div>
          </div>

          <div className="tu-grid2" style={{ marginTop: "18px" }}>
            <label className="tu-field">
              <Label>Industry</Label>
              <select
                className="tu-select"
                defaultValue="Select..."
                style={{ backgroundImage: CHEVRON_BG }}
              >
                {INDUSTRY.map((o) => (
                  <option key={o}>{o}</option>
                ))}
              </select>
            </label>
            <label className="tu-field">
              <Label>Budget Range</Label>
              <select
                className="tu-select"
                defaultValue="Select..."
                style={{ backgroundImage: CHEVRON_BG }}
              >
                {BUDGET.map((o) => (
                  <option key={o}>{o}</option>
                ))}
              </select>
            </label>
          </div>
          <p className="tu-help">
            Helps us route you to the right team faster.
          </p>

          <label className="tu-field tu-block">
            <span className="tu-label">Tell us what you're looking for</span>
            <textarea
              className="tu-area"
              placeholder="Goals, timeline, platforms, anything specific..."
            />
          </label>

          <button type="button" className="tu-submit">
            Send My Brief →
          </button>
        </div>

        {/* why */}
        <div className="tu-right">
          <div className="tu-rev" style={{ transitionDelay: "90ms" }}>
            <div className="tu-eyebrow">
              <span className="tu-eyebrow-line" />
              <span className="tu-eyebrow-t">Why brands send their brief</span>
            </div>
            <h2 className="tu-h2">
              You won't get a sales-pitch deck. You'll get a custom proposal.
            </h2>
          </div>
          {BENEFITS.map((b, i) => (
            <div
              key={b.t}
              className="tu-item tu-rev"
              style={{ transitionDelay: 170 + i * 90 + "ms" }}
            >
              <span className="tu-check" aria-hidden="true">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="3"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              </span>
              <div>
                <h3 className="tu-item-t">{b.t}</h3>
                <p className="tu-item-d">{b.d}</p>
              </div>
            </div>
          ))}

          <div
            className="tu-quote tu-rev"
            style={{ transitionDelay: 170 + BENEFITS.length * 90 + "ms" }}
          >
            <p className="tu-quote-t">
              SocialMelo cut our creator vetting time in half and we saw 3x the
              engagement of our previous agency in the first month.
            </p>
            <div className="tu-quote-by">
              <span className="tu-avatar" aria-hidden="true">
                JC
              </span>
              <div>
                <p className="tu-quote-name">Jordan Chen</p>
                <p className="tu-quote-role">Head of Brand, Loop &amp; Co.</p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="tu-bottombar" aria-hidden="true" />
    </section>
  );
}

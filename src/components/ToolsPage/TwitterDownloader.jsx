...
export default function TwitterDownloader() {
  ...
  const currentConfig = twitterConfig[format];
  ...
  const isValidTiktokUrl = (url) => {
    const tiktokRegex = /^https?:\/\/(www\.)?(twitter\.com|story\.twitter\.com|t\.twitter\.com)\/.+/;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (result.data.downloadUrl) {
        ...
        link.target = '_blank';
        link.download = `tiktok_${format}_${Date.now()}`;
  ...
  return (
    <>
      ...
      <TwitterMannual format={format} />
      <TwitterFeature format={format} />
      <TwitterFaq format = {format} />
    </>
  )
}
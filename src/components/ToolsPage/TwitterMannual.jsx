import './TwitterMannual.css';
import { twitterConfig } from '../config/TwitterConfig';

const TwitterMannual = ({ format = 'mp4' }) => {
  const config = twitterConfig[format].manual;

  const getTitleParts = () => {
    const title = config.title;

    if (title.includes('Twitter')) {
      const parts = title.split('Twitter');
      return {
        before: parts[0],
        highlight: 'Twitter ' + parts[1],
        after: ''
      };
    }

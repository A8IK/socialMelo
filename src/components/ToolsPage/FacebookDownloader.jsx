...
export default function FacebookDownloader() {
  ...
  const currentConfig = facebookConfig[format];
  ...
  const isValidFacebookUrl = (url) => {
    const facebookRegex = /^https?:\/\/(www\.)?(facebook\.com|story\.facebook\.com|t\.facebook\.com)\/.+/;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (result.data.downloadUrl) {
        ...
        link.target = '_blank';
        link.download = `facebook_${format}_${Date.now()}`;
  ...
  return (
    <>
      ...
      <FacebookMannual format={format} />
      <FacebookFeature format={format} />
      <FacebookFaq format = {format} />
    </>
  )
}
import './FacebookMannual.css';
import { facebookConfig } from '../config/FacebookConfig';

const FacebookMannual = ({ format = 'mp4' }) => {
  const config = facebookConfig[format].manual;

  const getTitleParts = () => {
    const title = config.title;

    if (title.includes('YouTube')) {
      const parts = title.split('YouTube');
      return {
        before: parts[0],
        highlight: 'YouTube ' + parts[1],
        after: ''
      };
    }
...
export default FacebookMannual;
